...
#define SENT_AR_RELEASE_MINOR_VERSION_C       4
#define SENT_AR_RELEASE_REVISION_VERSION_C    0
#define SENT_SW_MAJOR_VERSION_C               2
...
*/
static uint8 Sent_au8ChannelControllerMap[SENT_MAX_SUPPORTED_CHANNELS];
...
==================================================================================================*/
    ...
    #include "Sent_MemMap.h"
    #if (STD_ON == SENT_MULTICORE_SUPPORT)
        extern const Sent_ConfigType *Sent_Config[SENT_MAX_PARTITIONS];
    #else
...
void Sent_Init(const Sent_ConfigType * Config)
{
    ...
    else
    {
...
#endif /* SENT_PRECOMPILE_SUPPORT */
        {
    ...
    #if (SENT_MULTICORE_SUPPORT == STD_ON)
            {
                (void)Det_ReportError((uint16)SENT_MODULE_ID, (uint8)SENT_INSTANCE, (uint8)SENT_SID_INIT, (uint8)SENT_E_INIT_FAILED);
            }
            else
            {
    #else
            ...
            else
            {
                ...
                /* Initialize the channel->controller mapping by setting the initial value to CHANNEL_UNUSED (no mapping already done) */
                for (HwChnlOffset = 0; HwChnlOffset < SENT_MAX_SUPPORTED_CHANNELS; HwChnlOffset++)
                {
                    Sent_au8ChannelControllerMap[HwChnlOffset] = CHANNEL_UNUSED;
                }
                ...
                for(ControllerId=(uint8)0U; ControllerId < SENT_NUMBER_CONFIGURED_CONTROLLER; ControllerId++)
                {
                    ...
                    {
                        for(ChannelIndex = 0; ChannelIndex < SentController->ChnlConfigured; ChannelIndex++)
                        {
                             HwChnlOffset = SentController->CtrlConfig->CtrlConfig->ChnlConfig[ChannelIndex]->ChannelHwOffset;
                            /* Map configured hardware channel to logical channel */
                            if (Sent_apxConfig[CoreId]->ControllerConfig[ControllerId]->CtrHwID == ControllerId)
                            {
                                Sent_au8ChannelControllerMap[HwChnlOffset] = ControllerId;
                            }
                        }
                        Sent_Ipw_ControllerInit(SentController->CtrHwOffset, SentController->CtrlConfig);
                    }
                }
#if (SENT_DEV_ERROR_DETECT == STD_ON)
...
void Sent_DeInit(void)
{
    ...
    if (SENT_INIT != Sent_au8DriverStatus[CoreId])
    {
        ...
        (void)Det_ReportError((uint16)SENT_MODULE_ID, (uint8)SENT_INSTANCE, (uint8)SENT_SID_DEINIT, (uint8)SENT_E_UNINIT);
    }
    else
    {
#endif /* (SENT_DEV_ERROR_DETECT == STD_ON) */
        for (ControllerId = 0; ControllerId < SENT_NUMBER_CONFIGURED_CONTROLLER; ControllerId++)
        {
            ...
            if (NULL_PTR != SentController)
            {
                Sent_Ipw_ControllerDeInit(SentController->CtrHwOffset);
            }
        }
#if (SENT_DEV_ERROR_DETECT == STD_ON)
...
Std_ReturnType Sent_GetFastChannelMsgData(uint8 ChannelId)
{
#if (SENT_DEV_ERROR_DETECT == STD_ON)
    uint32  CoreId = 0;
#endif
    ...
    if (SENT_INIT != Sent_au8DriverStatus[CoreId])
    {
        ...
        (void)Det_ReportError((uint16)SENT_MODULE_ID, (uint8)SENT_INSTANCE, (uint8)SENT_SID_GETFASTCHANNELMSGDATA, (uint8)SENT_E_UNINIT);
    }
    else
    {
        /* Check for invalid channel */
        if (ChannelId >= SENT_NUMBER_CONFIGURED_CHANNELS)
        {
            /* Invalid channel */
            /* Report error to development error tracer */
            (void)Det_ReportError((uint16)SENT_MODULE_ID,(uint8)SENT_INSTANCE, (uint8)SENT_SID_GETFASTCHANNELMSGDATA, (uint8)SENT_E_INVALID_CHANNEL);
        }
        else
        {

#endif
            HwCtrlOffset  = Sent_au8ChannelControllerMap[ChannelId];
            RelVal = Sent_Ipw_GetFastChannelMsgData(HwCtrlOffset, ChannelId);
#if (SENT_DEV_ERROR_DETECT == STD_ON)
        }
    }
#endif /* (SENT_DEV_ERROR_DETECT == STD_ON) */
...
Std_ReturnType Sent_GetFastMsgData(void)
{
#if (SENT_DEV_ERROR_DETECT == STD_ON)
    uint32  CoreId = 0;
#endif
    ...
    if (SENT_INIT != Sent_au8DriverStatus[CoreId])
    {
        /* Sent driver has not been initialized yet */
        /* Report error to development error tracer */
        (void)Det_ReportError((uint16)SENT_MODULE_ID, (uint8)SENT_INSTANCE, (uint8)SENT_SID_GETFASTMSGDATA, (uint8)SENT_E_UNINIT);
    }
    else
    {
#endif
        RelVal = Sent_Ipw_GetFastMsgData();
#if (SENT_DEV_ERROR_DETECT == STD_ON)
    }
#endif /* (SENT_DEV_ERROR_DETECT == STD_ON) */
    return RelVal;
...
Std_ReturnType Sent_GetSerialChannelMsgData(uint8 ChannelId)
{
#if (SENT_DEV_ERROR_DETECT == STD_ON)
    uint32  CoreId = 0;
#endif
    ...
    if (SENT_INIT != Sent_au8DriverStatus[CoreId])
    {
        /* Sent driver has not been initialized yet */
        /* Report error to development error tracer */
        (void)Det_ReportError((uint16)SENT_MODULE_ID, (uint8)SENT_INSTANCE, (uint8)SENT_SID_GETSERIALCHANNELMSGDATA, (uint8)SENT_E_UNINIT);
    }
    else
    {
        /* Check for invalid channel */
        if (ChannelId >= SENT_NUMBER_CONFIGURED_CHANNELS)
        {
            /* Invalid channel */
            /* Report error to development error tracer */
            (void)Det_ReportError((uint16)SENT_MODULE_ID,(uint8)SENT_INSTANCE,(uint8)SENT_SID_GETSERIALCHANNELMSGDATA, (uint8)SENT_E_INVALID_CHANNEL);
        }
        else
        {
...
#endif
            HwCtrlOffset  = Sent_au8ChannelControllerMap[ChannelId];
            RelVal = Sent_Ipw_GetSerialChannelMsgData(HwCtrlOffset, ChannelId);
#if (SENT_DEV_ERROR_DETECT == STD_ON)
        }
    }
#endif /* (SENT_DEV_ERROR_DETECT == STD_ON) */
...
Std_ReturnType Sent_GetSerialMsgData(void)
{
#if (SENT_DEV_ERROR_DETECT == STD_ON)
    uint32  CoreId = 0;
#endif
    ...
    if (SENT_INIT != Sent_au8DriverStatus[CoreId])
    {
        /* Sent driver has not been initialized yet */
        /* Report error to development error tracer */
        (void)Det_ReportError((uint16)SENT_MODULE_ID, (uint8)SENT_INSTANCE, (uint8)SENT_SID_GETSERIALMSGDATA, (uint8)SENT_E_UNINIT);
    }
    else
    {
#endif
        RelVal = Sent_Ipw_GetSerialMsgData();
#if (SENT_DEV_ERROR_DETECT == STD_ON)
    }
#endif /* (SENT_DEV_ERROR_DETECT == STD_ON) */
    return RelVal;
...
void Sent_GetVersionInfo (Std_VersionInfoType * VersionInfo)
{
    ...
    {
        (void)Det_ReportError((uint16)SENT_MODULE_ID, (uint8)SENT_INSTANCE, (uint8)SENT_SID_GETVERSIONINFO, (uint8) SENT_E_INVALID_POINTER);
    }
    else
    {
#endif /* SENT_DEV_ERROR_DETECT == STD_ON */
