...
#define EMIOS_OCU_IP_AR_RELEASE_MINOR_VERSION_C        4
#define EMIOS_OCU_IP_AR_RELEASE_REVISION_VERSION_C     0
#define EMIOS_OCU_IP_SW_MAJOR_VERSION_C                2
...
*/
static const Emios_Ocu_Ip_ModuleConfigType * Emios_Ocu_Ip_spcxIpConfig;
...
*/
static Emios_Ocu_Ip_GlobalDrvStateType Emios_Ocu_Ip_sGlobalDrvState = EMIOS_OCU_IP_DRV_STATE_UNINIT;
...
*/
static Emios_Ocu_Ip_PinActionType Emios_Ocu_Ip_axPinAction[OCU_EMIOS_INSTANCE_COUNT][OCU_EMIOS_CHANNEL_COUNT] = {{(uint8)OCU_EMIOS_SET_LOW}};
...
*/
static uint16 Emios_Ocu_Ip_sau16MaxCounterVals[OCU_EMIOS_INSTANCE_COUNT][OCU_EMIOS_CHANNEL_COUNT];
...
Emios_Ocu_Ip_ChStateType Emios_Ocu_Ip_gaxChState[OCU_EMIOS_INSTANCE_COUNT][OCU_EMIOS_CHANNEL_COUNT] = {{{(boolean)FALSE, NULL_PTR, (uint16)OCU_EMIOS_INVALID_CHANNEL_NUM}}};
#endif
...
{
#if (STD_ON == OCU_DEV_ERROR_DETECT)
    DevAssert(ChNum < OCU_EMIOS_CHANNEL_COUNT);
    DevAssert(InstNum < OCU_EMIOS_INSTANCE_COUNT);
#endif
    if ((ChNum < OCU_EMIOS_CHANNEL_COUNT) && (InstNum < OCU_EMIOS_INSTANCE_COUNT))
    {
...
*
*/
void Emios_Ocu_Ip_Init(const Emios_Ocu_Ip_ModuleConfigType * const pEmiosIpConfig)
{
...
uint8   u8ModuleIdxUserMode = 0xFFU;    /* assigned eMIOS HW module index */
    ...
    if (NULL_PTR == Emios_Ocu_Ip_spcxIpConfig)
    {
        /* Save configuration pointer in a local variable */
        Emios_Ocu_Ip_spcxIpConfig = pEmiosIpConfig;
#if (STD_ON == OCU_DEV_ERROR_DETECT)
        DevAssert((0U < Emios_Ocu_Ip_spcxIpConfig->NumChannels) && (Emios_Ocu_Ip_spcxIpConfig->NumChannels <= OCU_NUM_HW_CHANNELS));
#endif
        /* Initialize each eMIOS configured channel */
        for (ChLoopIdx = 0U; ChLoopIdx < Emios_Ocu_Ip_spcxIpConfig->NumChannels; ++ChLoopIdx)
        {
            pEmiosChannelCfg = &(*Emios_Ocu_Ip_spcxIpConfig->pcxChannelsConfig)[ChLoopIdx];

            lu32CtrVal  = pEmiosChannelCfg->ControlValue;

            /* Get eMIOS channel and instance numbers */
...
#if (STD_ON == OCU_DEV_ERROR_DETECT)
            DevAssert((ChNum < OCU_EMIOS_CHANNEL_COUNT) && (InstNum < OCU_EMIOS_INSTANCE_COUNT));
#endif
            ...
            Emios_Ocu_Ip_sau16MaxCounterVals[InstNum][ChNum] = pEmiosChannelCfg->MaxCounterValue;
#endif
            ...
            Emios_Ocu_Ip_SetEdgePolarityBit(Emios_Ocu_Ip_gapcxBase[InstNum], ChNum, (1U == ((lu32CtrVal & OCU_EMIOS_PIN_STATE_MASK) >> OCU_EMIOS_PIN_STATE_SHIFT)) ? TRUE : FALSE);
...
#if ((STD_ON == OCU_GET_COUNTER_API) || (STD_ON == OCU_SET_RELATIVE_THRESHOLD_API) || (STD_ON == OCU_SET_ABSOLUTE_THRESHOLD_API))
                /* Store CounterBus Id */
                Emios_Ocu_Ip_au8CounterBusIdx[InstNum][ChNum] = Emios_Ocu_Ip_GetChannelNum(pEmiosChannelCfg->HwChannelCounterBus);
#endif
            }
            else
            {
        ...
        Emios_Ocu_Ip_sGlobalDrvState = EMIOS_OCU_IP_DRV_STATE_IDLE;
    }
}
...
void Emios_Ocu_Ip_DeInit(void)
{
...
#if (STD_ON == OCU_DEV_ERROR_DETECT)
    DevAssert(NULL_PTR != Emios_Ocu_Ip_spcxIpConfig);
#endif
    ...
    if (NULL_PTR != Emios_Ocu_Ip_spcxIpConfig)
    {
        ...
        Emios_Ocu_Ip_spcxIpConfig = NULL_PTR;
        Emios_Ocu_Ip_sGlobalDrvState = EMIOS_OCU_IP_DRV_STATE_UNINIT;
    }
}
...
void Emios_Ocu_Ip_StartChannel(uint8 InstNum, uint8 ChNum)
{
    ...
    if (    (EMIOS_OCU_IP_DRV_STATE_IDLE == Emios_Ocu_Ip_sGlobalDrvState) &&
...
Emios_Ocu_Ip_ReturnType Emios_Ocu_Ip_SetRelativeThreshold(uint8 InstNum, uint8 ChNum, Emios_Ocu_Ip_ValueType RelativeValue)
{
    ...
    Emios_Ocu_Ip_ReturnType RetVal  = OCU_EMIOS_CM_OUT_REF_INTERVAL;   /* Return value to see if the current counter has overrunned the postupdate CNT */
 ...
 *
 */
void Emios_Ocu_Ip_SetClockMode(Emios_Ocu_Ip_SelectPrescalerType Prescaler)
{
    uint8 InstNum;
    uint8 ChNum;
    uint8 ChLoopIdx;    /* Channel index used in the loop */
 ...
 *
 */
uint16 Emios_Ocu_Ip_GetMaxCounterValue(uint8 InstNum, uint8 ChNum)
{
    ...
    if (EMIOS_OCU_IP_DRV_STATE_IDLE == Emios_Ocu_Ip_sGlobalDrvState)
    {
        RetMaxCntVal = Emios_Ocu_Ip_sau16MaxCounterVals[InstNum][ChNum];
    }
    return RetMaxCntVal;
}
#endif
