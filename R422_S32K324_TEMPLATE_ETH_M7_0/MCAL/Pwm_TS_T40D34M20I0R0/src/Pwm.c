...
==================================================================================================*/
    ...
    /* Check if source file and StandardTypes.h are of the same AUTOSAR version */
        ...
        #error "AUTOSAR version numbers of Pwm.c and StandardTypes.h are different."
    #endif
...
void Pwm_SetClockMode(Pwm_PrescalerType Prescaler)
{
    ...
    for (Index = 0; Index < Pwm_aState[CoreId].PwmConfig->NumInstances; Index++)
    {
        Pwm_Ipw_SetClockMode(Prescaler, &((*Pwm_aState[CoreId].PwmConfig->PwmInstancesConfig)[Index]));
    }
#else
    for(Index = 0; Index < Pwm_aState[CoreId].PwmConfig->NumChannels; Index++)
    {
        Pwm_Ipw_SetClockMode(Prescaler, &((*Pwm_aState[CoreId].PwmChannelConfigs)[Index].IpwChannelCfg));
    }
#endif
                ...
                switch(Pwm_eTargetPowerState[CoreId])
                {
                    ...
                    case PWM_FULL_POWER:
                        ...
                        if((Std_ReturnType)E_OK == RetVal)
                        {
                            ...
                            for (Index = 0; Index < Pwm_aState[CoreId].PwmConfig->NumInstances; Index ++)
                            {
                                Pwm_Ipw_SetPowerState(&((*Pwm_aState[CoreId].PwmConfig->PwmInstancesConfig)[Index]), Pwm_eTargetPowerState[CoreId]);
                            }
#else
                            for(Index = 0; Index < Pwm_aState[CoreId].PwmConfig->NumChannels; Index++)
                            {
                                Pwm_Ipw_SetPowerState(Pwm_eTargetPowerState[CoreId], &((*Pwm_aState[CoreId].PwmChannelConfigs)[Index].IpwChannelCfg));
                            }
#endif
                            *Result = PWM_SERVICE_ACCEPTED;
