...
#define RM_XBIC_IP_AR_RELEASE_MINOR_VERSION_C         4
#define RM_XBIC_IP_AR_RELEASE_REVISION_VERSION_C      0
#define RM_XBIC_IP_SW_MAJOR_VERSION_C                 2
 ...
 *
 * */
void Xbic_Ip_IntegrityCheckerGetErrorStatus(uint32 xbicInstance, Xbic_Ip_ErrorInfoType * pErrorInfo)
{
    uint32 u32ErrValue;
    uint8  u8master;
    uint8  u8slave;
    ...
    pErrorInfo->bErrorStatusValid         = Xbic_Ip_ConvertToBoolean((u32ErrValue & XBIC_ESR_VLD_MASK) >> XBIC_ESR_VLD_SHIFT); 
    /* Data Phase Slave Port Error */
    for (u8slave = 0U; u8slave < XBIC_IP_NUM_SLAVE; u8slave++)
    {
        pErrorInfo->aDataPhaseSlavePortError[u8slave] = Xbic_Ip_ConvertToBoolean(((u32ErrValue & (XBIC_ESR_DPSE0_MASK >> u8slave)) >> (XBIC_ESR_DPSE0_SHIFT - u8slave))); 
    }
    /* Data Phase Master Port Error */
    for (u8master = 0U; u8master < XBIC_IP_NUM_MASTER; u8master++)
    {
        pErrorInfo->aDataPhaseMasterPortError[u8master] = Xbic_Ip_ConvertToBoolean(((u32ErrValue & (XBIC_ESR_DPME0_MASK >> u8master)) >> (XBIC_ESR_DPME0_SHIFT - u8master))); 
    }
    pErrorInfo->masterID                  = (uint8)((u32ErrValue & XBIC_ESR_MST_MASK) >> XBIC_ESR_MST_SHIFT);
    pErrorInfo->slavePort                 = (uint8)((u32ErrValue & XBIC_ESR_SLV_MASK) >> XBIC_ESR_SLV_SHIFT);
    pErrorInfo->synError                  = (uint8)((u32ErrValue & XBIC_ESR_SYN_MASK) >> XBIC_ESR_SYN_SHIFT);

#ifdef XBIC_IP_ENABLE_USER_MODE_SUPPORT
    pErrorInfo->errAddressDetect = OsIf_Trusted_Call_Return1param(Xbic_Ip_ReadRegister,XBIC_IP_EAR_ADDR32(xbicInstance));
#else
    pErrorInfo->errAddressDetect = Xbic_Ip_ReadRegister(XBIC_IP_EAR_ADDR32(xbicInstance));
#endif
