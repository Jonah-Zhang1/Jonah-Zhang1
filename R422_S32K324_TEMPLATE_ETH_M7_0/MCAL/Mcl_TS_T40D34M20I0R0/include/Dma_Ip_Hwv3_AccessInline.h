...
#define DMA_IP_HWV3_ACCESSINLINE_SW_MAJOR_VERSION                2
...
static inline void hwv3AccInlineDmaCh_CmdSwReqStart(Dma_Ip_TcdRegType * ptTcd)
{
    ...
    reg = ((reg & (~(DMA_IP_TCD_CSR_START_MASK))) | (DMA_IP_TCD_CSR_START(TRUE)));
    ptTcd->reg_CSR = reg;
}
...
static inline void hwv3AccInlineDmaCh_SetRequest_SourceMux(volatile Dma_Ip_MuxRegType * pMuxChCfg, const uint32 Value)
{
...
static inline void hwv3AccInlineDmaCh_SetRequest_EnTriggerMux(volatile Dma_Ip_MuxRegType * pMuxChCfg, uint32 Value)
{
    /* Access DMAMUX_CHCFG_TRIG register */
    Dma_Ip_MuxRegType reg = *pMuxChCfg;
    *pMuxChCfg = ((uint8)(reg & (~(DMAMUX_CHCFG_TRIG_MASK))) | (DMAMUX_CHCFG_TRIG(Value)));
}
static inline void hwv3AccInlineDmaCh_SetRequest_EnHwRequestMux(volatile Dma_Ip_MuxRegType * pMuxChCfg, const uint32 Value)
{
...
static inline void hwv3AccInlineDmaCh_SetSource_Address(Dma_Ip_TcdRegType * ptTcd, const uint32 Value)
{
    ...
    uint32 reg = ptTcd->reg_SADDR;
    ptTcd->reg_SADDR = ((reg & (~(DMA_IP_TCD_SADDR_SADDR_MASK))) | (DMA_IP_TCD_SADDR_SADDR(Value)));
}
...
static inline void hwv3AccInlineDmaCh_SetSource_SignedLastAddrAdj(Dma_Ip_TcdRegType * ptTcd, const uint32 Value)
{
    ...
    uint32 reg = ptTcd->reg_SLAST_SDA;
    ptTcd->reg_SLAST_SDA = ((reg & (~(DMA_IP_TCD_SLAST_SDA_SLAST_SDA_MASK))) | (DMA_IP_TCD_SLAST_SDA_SLAST_SDA(Value)));
}
...
static inline void hwv3AccInlineDmaCh_SetDestination_Address(Dma_Ip_TcdRegType * ptTcd, const uint32 Value)
{
    ...
    uint32 reg = ptTcd->reg_DADDR;
    ptTcd->reg_DADDR = ((reg & (~(DMA_IP_TCD_DADDR_DADDR_MASK))) | (DMA_IP_TCD_DADDR_DADDR(Value)));
}
...
static inline void hwv3AccInlineDmaCh_SetDestination_LastAddrAdj(Dma_Ip_TcdRegType * ptTcd, const uint32 Value)
{
    ...
    uint32 reg = ptTcd->reg_DLAST_SGA;
    ptTcd->reg_DLAST_SGA = ((reg & (~(DMA_IP_TCD_DLAST_SGA_DLAST_SGA_MASK))) | (DMA_IP_TCD_DLAST_SGA_DLAST_SGA(Value)));
}
...
#if (STD_ON == DMA_IP_STORE_DST_ADDR_IS_AVAILABLE)
static inline void hwv3AccInlineDmaCh_GetStoreDstAddress(const Dma_Ip_TcdRegType * ptTcd, uint32 * const retValue)
{
...
static inline void hwv3AccInlineDmaCh_SetControlAndStatus(Dma_Ip_TcdRegType * ptTcd, const Dma_Ip_ScatterGatherConfigType * const pxLocScatterGather)
{
    ...
    reg = ((uint16)(reg & (~(DMA_IP_TCD_CSR_ESDA_MASK))) | (DMA_IP_TCD_CSR_ESDA(FALSE)));
    ptTcd->reg_CSR= reg;
}
