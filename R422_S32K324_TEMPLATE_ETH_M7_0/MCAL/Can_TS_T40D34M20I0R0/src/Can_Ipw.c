...
#ifdef CAN_TS_STM_USED
    #include "Stm_Ip.h"
#endif

#ifdef CAN_TS_MAC_USED
    #include "Gmac_Ip.h"
#endif
/*==================================================================================================
...
#define CAN_IPW_SW_MAJOR_VERSION_C             2
#define CAN_IPW_SW_MINOR_VERSION_C             0
#define CAN_IPW_SW_PATCH_VERSION_C             0
/**@}*/
...
#if (CAN_TS_EXTERNAL_TIMER_USED == STD_ON)
...
#if ((CAN_TIMESTAMP_ENABLE == STD_ON) && ((CAN_TX_POLLING_SUPPORT == STD_ON) || (FLEXCAN_IP_MB_INTERRUPT_SUPPORT == STD_ON)))
static boolean Can_Ipw_abIsTxPduReady[CAN_CONTROLLER_CONFIG_COUNT][CAN_HWMB_COUNT];
#endif /* ((CAN_TIMESTAMP_ENABLE == STD_ON) && ((CAN_TX_POLLING_SUPPORT == STD_ON) || (FLEXCAN_IP_MB_INTERRUPT_SUPPORT == STD_ON))) */
...
#if (CAN_TS_EXTERNAL_TIMER_USED == STD_ON)
#if (CAN_NO_CACHE_NEEDED == STD_ON)
#define CAN_START_SEC_VAR_INIT_32_NO_CACHEABLE
#else
...
static volatile uint32 Can_Ipw_u32CurrentCompareVal = CAN_TS_STM_TARGETTIME;

#if (CAN_NO_CACHE_NEEDED == STD_ON)
#define CAN_STOP_SEC_VAR_INIT_32_NO_CACHEABLE
#else
...
#if ((CAN_TIMESTAMP_ENABLE == STD_ON) && (CAN_ENHANCED_FIFO_ENABLED == STD_ON) && (FLEXCAN_IP_FEATURE_HAS_DMA_ENABLE == STD_ON))
#if (CAN_NO_CACHE_NEEDED == STD_ON)
#define CAN_START_SEC_VAR_CLEARED_32_NO_CACHEABLE
#else
#define CAN_START_SEC_VAR_CLEARED_32
...
static uint32 Can_Ipw_u32MbTransferredIndex[CAN_CONTROLLER_CONFIG_COUNT];
...
#endif /* (CAN_NO_CACHE_NEEDED == STD_ON) */
#include "CAN_MemMap.h"
#endif /* ((CAN_TIMESTAMP_ENABLE == STD_ON) && (CAN_ENHANCED_FIFO_ENABLED == STD_ON) && (FLEXCAN_IP_FEATURE_HAS_DMA_ENABLE == STD_ON)) */
...
static uint32 Can_Ipw_GetTimeElapsed(uint32 counterValue, uint32 compareValue)
{
    ...
    if (tempValue > CAN_TS_STM_TARGETTIME)
    {
        /* the timestamp captured was not in current compare period */
        returnValue = CAN_TS_STM_TARGETTIME + (uint32)1U;
    }
    else
    {
        returnValue = CAN_TS_STM_TARGETTIME - tempValue;
    }
...
static void Can_Ipw_GetTimestamp(Can_TimeStampType * TimestampPtr, uint32 Value)
{
    ...
    Gmac_Ip_GetSysTime(CAN_TS_MAC_INSTANCE, &MacSysTime);
    ...
    CompareTemp = Stm_Ip_GetCompareValue(CAN_TS_STM_INSTANCE, CAN_TS_STM_CHANNEL);
    ...
    if ((TimestampPtr->nanoseconds) > CAN_TS_STM_TARGETTIME)
    {
        ...
        else
        {
            ...
            if ((TimestampPtr->nanoseconds) >= CAN_TS_STM_TARGETTIME)
            {
                ...
                /* case 2: For each second elapsed, a STM interrupt will be triggered and STM driver will update next compare value ( = current timebase value + CAN_TS_STM_TARGETTIME) but the timebase will be increased automatically even if STM interrupt triggered,
                it means that CurrCompareValTemp - PreCompareValTemp > CAN_TS_STM_TARGETTIME because it depends on stm's performence for processing its ISR, Then if a flexcan timestamp was captured in that time, the nanosecond will be 0 */
                /* Example:
                *          10s                          11s
                *-|---------------------|-----|----------------------|---------------------------->
    ...
    else if ((TimestampPtr->nanoseconds) == CAN_TS_STM_TARGETTIME)
    {
        TimestampPtr->nanoseconds = 0U;
        (TimestampPtr->seconds)++;
    }
    ...
    TimestampPtr->nanoseconds = (uint32)(((uint64)(TimestampPtr->nanoseconds) * (uint64)CAN_TS_ACCURACY) / (uint64)1000UL);
#endif /* CAN_TS_STM_USED */
}
...
void Can_TimestampTimerOverflowCallback(void)
{
    ...
    Can_Ipw_u32CurrentCompareVal = Stm_Ip_GetCompareValue(CAN_TS_STM_INSTANCE, CAN_TS_STM_CHANNEL);
}
#endif /* (CAN_TS_EXTERNAL_TIMER_USED == STD_ON) */
#endif /* (CAN_TIMESTAMP_ENABLE == STD_ON) */
/*==================================================================================================
...
Std_ReturnType Can_Ipw_SetControllerToStartMode(const Can_ControllerConfigType * Can_pControllerConfig)
{
    ...
    {
        if (Can_Ipw_u32CurrentCompareVal != Stm_Ip_GetCompareValue(CAN_TS_STM_INSTANCE, CAN_TS_STM_CHANNEL))
        {
            Can_Ipw_u32CurrentCompareVal = Stm_Ip_GetCompareValue(CAN_TS_STM_INSTANCE, CAN_TS_STM_CHANNEL);
        }
    }
    ...
    #if ((CAN_TIMESTAMP_ENABLE == STD_ON) && ((CAN_TX_POLLING_SUPPORT == STD_ON) || (FLEXCAN_IP_MB_INTERRUPT_SUPPORT == STD_ON)))
        for (MbIdx = 0; MbIdx < CAN_HWMB_COUNT; MbIdx++)
        {
...
void Can_Ipw_GetCurrentTime(Can_TimeStampType* timeStampPtr)
{
#ifdef CAN_TS_MAC_USED
    Gmac_Ip_TimestampType MacSysTime;
...
#ifdef CAN_TS_STM_USED
    timeStampPtr->nanoseconds = (uint32)(((uint64)(Can_Ipw_GetTimeElapsed(Stm_Ip_GetCounterValue(CAN_TS_STM_INSTANCE), Stm_Ip_GetCompareValue(CAN_TS_STM_INSTANCE, CAN_TS_STM_CHANNEL))) * (uint64)CAN_TS_ACCURACY) / (uint64)1000UL);
    timeStampPtr->seconds = Can_Ipw_u32Seconds;
#endif
}
...
void Can_Ipw_SetCurrentTime(const Can_TimeStampType* timeStampPtr)
{
    Can_Ipw_u32Seconds = timeStampPtr->seconds;
}
#endif /* (CAN_TS_EXTERNAL_TIMER_USED == STD_ON) */

void Can_Ipw_EnableEgressTimeStamp
(
    const Can_ControllerConfigType * Can_pControllerConfig,
    const Can_HwObjectConfigType * Can_pHwObjectConfig
)
...
Std_ReturnType Can_Ipw_GetEgressTimeStamp
(
    const Can_ControllerConfigType * Can_pControllerConfig,
    const Can_HwObjectConfigType * Can_pHwObjectConfig,
...
)
{
    ...
    {
        (void)Det_ReportError((uint16)CAN_IPW_MODULE_ID, CAN_INSTANCE, CAN_SID_GET_EGRESS_TIMESTAMP, CAN_E_PARAM_LPDU);
    }
    ...
    (void)timeStampPtr;
    ReturnValue = (Std_ReturnType)E_OK;
#endif /* ((CAN_TX_POLLING_SUPPORT == STD_ON) || (CAN_MB_INTERRUPT_SUPPORT == STD_ON)) */
    return ReturnValue;
}

void Can_Ipw_GetIngressTimeStamp
(
    const Can_ControllerConfigType * Can_pControllerConfig,
    const Can_HwObjectConfigType * Can_pHwObjectConfig,
    Can_TimeStampType* timeStampPtr
