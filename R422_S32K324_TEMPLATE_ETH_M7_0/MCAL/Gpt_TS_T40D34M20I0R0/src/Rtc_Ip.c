...
#define RTC_IP_AR_RELEASE_MINOR_VERSION_C     4
#define RTC_IP_AR_RELEASE_REVISION_VERSION_C  0
#define RTC_IP_SW_MAJOR_VERSION_C             2
...
static inline uint32 Rtc_Ip_GetStatusInterrupt(uint8 instance, Rtc_Ip_InterruptType interruptMode);
#endif
...
static inline void Rtc_Ip_ClearStatusFlags(RTC_Type * const base, uint32 mask);
...
static inline void Rtc_Ip_ApiInterruptEnable(RTC_Type * const base, boolean enable);
static inline void Rtc_Ip_RtcInterruptEnable(RTC_Type * const base, boolean enable);
static inline void Rtc_Ip_RollOverInterruptEnable(RTC_Type * const base, boolean enable);
...
static inline void Rtc_Ip_SetCompareValue(RTC_Type * const base, uint32 value);
...
void Rtc_Ip_ConfigureNonSupervisorAccess(void);
#endif
...
uint32 Rtc_Ip_GetStatusFlags(uint8 instance, Rtc_Ip_InterruptType interruptMode)
{
    uint32 flag = 0U;
    /* Check all case interrupt mode */
    switch(interruptMode)
    {
        case RTC_IP_COUNTER_INTERRUPT:
        {
            flag = ((rtcBase[instance]->RTCS) & (RTC_RTCS_RTCF_MASK )) >> RTC_RTCS_RTCF_SHIFT;
        }
        break;
        case RTC_IP_API_INTERRUPT:
        {
            flag = ((rtcBase[instance]->RTCS) & (RTC_RTCS_APIF_MASK)) >> RTC_RTCS_APIF_SHIFT;
        }
        break;
        case RTC_IP_ROLLOVER_INTERRUPT:
        {
            flag = ((rtcBase[instance]->RTCS) & (RTC_RTCS_ROVRF_MASK)) >> RTC_RTCS_ROVRF_SHIFT;
        }
        break;
        default:
        {
...
{
    uint32 flag = 0U;
    /* Get GetStatusInterrupt */
     switch(interruptMode)
    {
        case RTC_IP_COUNTER_INTERRUPT:
        {
            flag = ((rtcBase[instance]->RTCC) & (RTC_RTCC_RTCIE_MASK)) >> RTC_RTCC_RTCIE_SHIFT;
        }
        break;
        case RTC_IP_API_INTERRUPT:
        {
            flag = ((rtcBase[instance]->RTCC) & (RTC_RTCC_APIIE_MASK)) >> RTC_RTCC_APIIE_SHIFT;
        }
        break;
        case RTC_IP_ROLLOVER_INTERRUPT:
        {
            flag = ((rtcBase[instance]->RTCC) & (RTC_RTCC_ROVREN_MASK)) >> RTC_RTCC_ROVREN_SHIFT;
        }
        break;
        default:
        {
...
static inline void Rtc_Ip_EnableCounter(RTC_Type * const base, boolean enable)
{
    SchM_Enter_Gpt_GPT_EXCLUSIVE_AREA_04();
    {
        ...
        if (TRUE == enable)
        {
            base->RTCC |= (uint32)(1UL << RTC_RTCC_CNTEN_SHIFT);
        }
        else
        {
            base->RTCC &= ~(uint32)(1UL << RTC_RTCC_CNTEN_SHIFT);
...
static inline void Rtc_Ip_ApiInterruptEnable(RTC_Type * const base, boolean enable)
{
    SchM_Enter_Gpt_GPT_EXCLUSIVE_AREA_07();
    {
        ...
        if (TRUE == enable)
        {
            base->RTCC |= (uint32)(1UL << RTC_RTCC_APIIE_SHIFT);
        }
        else
        {
            base->RTCC &= ~(uint32)(1UL << RTC_RTCC_APIIE_SHIFT);
...
static inline void Rtc_Ip_RtcInterruptEnable(RTC_Type * const base, boolean enable)
{
    SchM_Enter_Gpt_GPT_EXCLUSIVE_AREA_10();
    {
        ...
        if (TRUE == enable)
        {
            base->RTCC |= (uint32)(1UL << RTC_RTCC_RTCIE_SHIFT);
        }
        else
        {
            base->RTCC &= ~(uint32)(1UL << RTC_RTCC_RTCIE_SHIFT);
...
static inline void Rtc_Ip_RollOverInterruptEnable(RTC_Type * const base, boolean enable)
{
    ...
    {
        /* RollOverInterruptEnable */
        if (TRUE == enable)
        {
            base->RTCC |= (uint32)(1UL << RTC_RTCC_ROVREN_SHIFT);
        }
        else
        {
            base->RTCC &= ~(uint32)(1UL << RTC_RTCC_ROVREN_SHIFT);
...
static uint32 Rtc_Ip_TicksToSeconds(uint8 instance, uint32 ticks)
{
    ...
    totalTicks = (uint64)(((uint64)Rtc_Ip_u32ChState[instance].rolloverCount * (uint64)MAX_32BIT) + \
                           (uint64)ticks - (uint64)Rtc_Ip_u32ChState[instance].timeOffset);
    seconds = totalTicks / (uint64)Rtc_Ip_u32ChState[instance].ticksPerSecond;
    /* Approximate if conversion of seconds from ticks is less than tolerance to not lose precision */
    if( (totalTicks % (uint64)Rtc_Ip_u32ChState[instance].ticksPerSecond) >= (((uint64)Rtc_Ip_u32ChState[instance].ticksPerSecond - \
                                                                               (uint64)RTC_IP_PPM_TOLERANCE )) )
    {
        /* Increment seconds because value was truncated by div operation */
        seconds++;
    }

    returnValue = (uint32)(seconds & MAX_32BIT);
...
static uint32 Rtc_Ip_SecondsToTicks(uint8 instance, uint32 seconds)
{
    ...
    ticks = (uint64)(((uint64)seconds * (uint64)Rtc_Ip_u32ChState[instance].ticksPerSecond) + \
                       (uint64)Rtc_Ip_u32ChState[instance].timeOffset - ((uint64)Rtc_Ip_u32ChState[instance].rolloverCount * MAX_32BIT));


    returnValue = (uint32)(ticks & MAX_32BIT);
    return returnValue;
}
    ...
    base = rtcBase[instance];
            ...
            if((FALSE != Rtc_Ip_u32ChState[instance].alarmInit) && (TRUE == Rtc_Ip_u32ChState[instance].alarmIntEnable))
            {
                if ((0UL < Rtc_Ip_u32ChState[instance].numberOfRepeats) || (TRUE == Rtc_Ip_u32ChState[instance].repeatForever))
                {
                    ...
                    {
                        Rtc_Ip_SetCompareValue(base, newAlarmTime);
...
static void Rtc_Ip_ProcessInterrupt(uint8 instance)
{
    RTC_Type * base;
    base = rtcBase[instance];
    uint32 apiInterruptFlag = 0U;
    uint32 apiInterruptEnable = 0U;
    uint32 rtcInterruptFlag = 0U;
    uint32 rtcInterruptEnable = 0U;
    uint32 rollOverInterruptFlag = 0U;
    uint32 rollOverInterruptEnable = 0U;


    /*Check the status flag register and interrupts enable register*/
    apiInterruptFlag = Rtc_Ip_GetStatusFlags(instance, RTC_IP_API_INTERRUPT);
    apiInterruptEnable = Rtc_Ip_GetStatusInterrupt(instance, RTC_IP_API_INTERRUPT);
    rtcInterruptFlag = Rtc_Ip_GetStatusFlags(instance, RTC_IP_COUNTER_INTERRUPT);
    rtcInterruptEnable = Rtc_Ip_GetStatusInterrupt(instance, RTC_IP_COUNTER_INTERRUPT);
    rollOverInterruptFlag = Rtc_Ip_GetStatusFlags(instance, RTC_IP_ROLLOVER_INTERRUPT);
    rollOverInterruptEnable = Rtc_Ip_GetStatusInterrupt(instance, RTC_IP_ROLLOVER_INTERRUPT);

    /*Clear interrupt flags*/
    Rtc_Ip_ClearStatusFlags(base, (RTC_RTCS_RTCF_MASK | RTC_RTCS_APIF_MASK | RTC_RTCS_ROVRF_MASK));
    Rtc_Ip_u32CounterValueInterrupt = Rtc_Ip_u32TargetValue + Rtc_Ip_GetCounterRegister(instance);
    if ((1U == apiInterruptFlag) && (1U == apiInterruptEnable))
    {
        if((RTC_IP_CH_MODE_ONESHOT == Rtc_Ip_u32ChState[instance].channelMode))
        {
            Rtc_Ip_StopTimer(instance);
        }
        /* Call upper layer handler */
        if((TRUE == Rtc_Ip_u32ChState[instance].chInit) && \
            (NULL_PTR != Rtc_Ip_u32ChState[instance].callback))
        {
            Rtc_Ip_u32ChState[instance].callback(Rtc_Ip_u32ChState[instance].callbackParam);
        }
    }
    /* Check if an alarm has occurred */
    else if((1U == rtcInterruptFlag) && (1U == rtcInterruptEnable))
    {       
        /* Process AlarmInterrupt */
        Rtc_Ip_ProcessAlarmInterrupt(instance);
    }
    else if ((1U == rollOverInterruptFlag) && (1U == rollOverInterruptEnable))
    {
...
void Rtc_Ip_Init(uint8 instance, const Rtc_Ip_ConfigType *config)
{
...
#endif
#if (STD_ON == RTC_IP_ENABLE_USER_MODE_SUPPORT)
    OsIf_Trusted_Call(Rtc_Ip_ConfigureNonSupervisorAccess);
#endif
    RTC_Type * base;
    base = rtcBase[instance];

#if (RTC_IP_STANDBY_WAKEUP_SUPPORT == STD_ON)
    /* Check if RTCC_CNTEN is 0x0UL or not  */
    if(0x0UL == ((base->RTCC & RTC_RTCC_CNTEN_MASK) >> RTC_RTCC_CNTEN_SHIFT))
    {
#endif
    ...
    {
    /* Clear status flags without APIF flag*/
    Rtc_Ip_ClearStatusFlags(base, (RTC_RTCS_RTCF_MASK | RTC_RTCS_ROVRF_MASK));
    }
#endif
...
void Rtc_Ip_StopTimer(uint8 instance)
{
    ...
    Rtc_Ip_ClearStatusFlags(base, RTC_RTCS_APIF_MASK);
}
...
void Rtc_Ip_EnableInterrupt(uint8 instance, Rtc_Ip_InterruptType interruptMode)
{
     ...
     switch(interruptMode)
    {
        case RTC_IP_COUNTER_INTERRUPT:
        {
            /* Enable RTC interrupt req */
            Rtc_Ip_RtcInterruptEnable(base, TRUE);
        }
        break;
        case RTC_IP_API_INTERRUPT:
        {
            /* Enable API interrupt req */
            Rtc_Ip_ApiInterruptEnable(base, TRUE);
        }
        break;
        case RTC_IP_ROLLOVER_INTERRUPT:
        {
            /* Enable API interrupt req */
            Rtc_Ip_RollOverInterruptEnable(base, TRUE);
        }
        break;
        default:
            /* This case should not be entered */
...
void Rtc_Ip_DisableInterrupt(uint8 instance, Rtc_Ip_InterruptType interruptMode)
{
     ...
     switch(interruptMode)
    {
        ...
        {
            /* Clear interrupt flag */
            Rtc_Ip_ClearStatusFlags(base, RTC_RTCS_RTCF_MASK);
            /* Enable RTC interrupt req */
            Rtc_Ip_RtcInterruptEnable(base, FALSE);
        }
        break;
        case RTC_IP_API_INTERRUPT:
        {
            ...
            Rtc_Ip_ClearStatusFlags(base, RTC_RTCS_APIF_MASK);
            /* Enable API interrupt req */
            Rtc_Ip_ApiInterruptEnable(base, FALSE);
        }
        break;
        case RTC_IP_ROLLOVER_INTERRUPT:
        {
            /* Clear interrupt flag */
            Rtc_Ip_ClearStatusFlags(base, RTC_RTCS_ROVRF_MASK);
            /* Enable API interrupt req */
            Rtc_Ip_RollOverInterruptEnable(base, FALSE);
        }
        break;
        default:
            /* This case should not be entered */
...
{
    Rtc_Ip_StatusType returnValue = RTC_IP_ERROR;
...
{
    Rtc_Ip_StatusType status = RTC_IP_ERROR;
#if RTC_IP_DEV_ERROR_DETECT == STD_ON
    DevAssert(RTC_INSTANCE_COUNT > instance);
#endif
